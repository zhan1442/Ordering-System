...
def order_create(request):
    ...
    if request.method == 'POST':
        ...
        if orderform.is_valid():
            ...
            order_obj = Order(name=drug, brand=brand, quantity=quantity, strength=strength, doctor_name=doctor, doctor_email=doctorEmail, comment=comment, client_obj=user)
            order_obj.save()
    ...
    if request.method == 'POST':
        ...
        if  drugformset.is_valid():#orderform.is_valid()
            ...
            for drugform in drugformset:
                ...
                drug_obj = Drug(name=name, brand=brand, strength=strength, quantity=quantity, order_obj=order_obj)
                drug_obj.save()
